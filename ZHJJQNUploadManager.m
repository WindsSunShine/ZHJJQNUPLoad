...
@interface ZHJJQNUploadManager ()

@property (nonatomic, assign) NSInteger index;

@end

@implementation ZHJJQNUploadManager


// 创建单例
+(instancetype)sharedInstance {
    
    static ZHJJQNUploadManager * manager = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        manager = [[ZHJJQNUploadManager alloc]init];
        manager.index = 0;
...
-(void)registerWithScope:(NSString *)scrop accessKey:(NSString *)accessKey secretKey:(NSString *)secretKey {
...
// 自定义过期时间的
-(void)registerWithScope:(NSString *)scrop accessKey:(NSString *)accessKey secretKey:(NSString *)secretKey liveTime:(NSInteger)liveTime {
